...
def hierarchical_packing(stock_dims: Tuple[float, float, float],
    ...
    sub_blocks = []
    scrap_volumes = []  # Track scrap volume for each sub-block
    ...
    for i, sb in enumerate(sub_blocks):
        ...
        if not fitting_part:
            # Track as 100% scrap if no fitting part found
            sub_block_volume = sb.dimensions[0] * sb.dimensions[1] * sb.dimensions[2]
            scrap_volumes.append({
                'sub_block_index': i,
                'sub_block_volume': sub_block_volume,
                'parts_volume': 0,
                'scrap_volume': sub_block_volume,
                'scrap_percentage': 100.0,
                'sub_block_dims': sb.dimensions
            })
            if verbose:
                print(f"    No fitting part found - skipping")
                print(f"    Sub-block volume: {sub_block_volume:,.0f} mm³")
                print(f"    Scrap volume: {sub_block_volume:,.0f} mm³ (100.00%)")
        ...
        # Calculate scrap volume for this sub-block
        sub_block_volume = sb.dimensions[0] * sb.dimensions[1] * sb.dimensions[2]
        parts_volume_in_subblock = len(sub_parts) * fit_geom.volume
        scrap_volume = sub_block_volume - parts_volume_in_subblock
        scrap_percentage = (scrap_volume / sub_block_volume) * 100 if sub_block_volume > 0 else 0

        scrap_volumes.append({
            'sub_block_index': i,
            'sub_block_volume': sub_block_volume,
            'parts_volume': parts_volume_in_subblock,
        ...
        if verbose:
            ...
            print(f"    Sub-block volume: {sub_block_volume:,.0f} mm³")
            print(f"    Parts volume: {parts_volume_in_subblock:,.0f} mm³")
            print(f"    Scrap volume: {scrap_volume:,.0f} mm³ ({scrap_percentage:.2f}%)")
    ...
    return primary_result, sub_blocks, sub_block_results, bounded_region, scrap_volumes
...
def main():
    ...
    if args.interactive:
        ...
        stock_dims = (stock.length, stock.width, stock.height)
        parts_to_pack = {name: PART_SPECS[name] for name in PART_SPECS if name in ["G14","G15","G2","G4"]}  # Use all parts G1-G56
...
# Replace the section from "hier_result = hierarchical_packing..." to the end
    ...
    for idx, primary_part_name in enumerate(parts_to_test, 1):
        ...
        for merging_plane_order in merging_plane_order_set:
            ...
            try:
                ...
                primary_result, sub_blocks, sub_block_results, bounded_region, scrap_volumes = hier_result
                ...
                # Calculate total scrap volume
                total_scrap_volume = sum(sv['scrap_volume'] for sv in scrap_volumes)
                ...
                stock_geom = create_stock_geometry(*stock_dims)
                geometries = [(stock_geom, "Stock", "black", 0.05)]

                # Add primary parts with green border
                for part in primary_result.placed_parts:
                    label = f"{config_name}_{part.part_id}"
                    geometries.append((part.geometry, label, "green", 0.6))

                # Add bounding box wireframe (green for desired shape border)
                ...
                geometries.append((bbox_geom, "BoundingBox", "green", 0.1))

                # Add sub-block wireframes (red for scrap blocks)
                for i, sb in enumerate(sub_blocks):
                    ...
                    # Get scrap info for this sub-block
                    scrap_info = scrap_volumes[i]
                    scrap_label = f"Scrap_SB{i+1}_{scrap_info['scrap_volume']:,.0f}mm³_{scrap_info['scrap_percentage']:.1f}%"
                    geometries.append((sb_geom, scrap_label, "red", 0.15))

                # Create title with scrap breakdown showing dimensions
                scrap_summary = " | ".join([f"SB{sv['sub_block_index']+1}: {sv['sub_block_dims'][0]:.0f}mm*{sv['sub_block_dims'][1]:.0f}mm*{sv['sub_block_dims'][2]:.0f}mm"
                                            for sv in scrap_volumes])
                filename = f"hierarchical_{config_name}_step1_subblocks"
                title = f"Step 1 ({config_name}): {len(primary_result.placed_parts)} {primary_part_name} + {len(sub_blocks)} scrap blocks | Scrap: {scrap_summary}"
                exporter.export_combined(geometries, filename, title)
                if verbose:
                    print(f"    Saved: {filename}.html")

                # Visualization 2: Complete hierarchical packing with filled sub-blocks
                geometries2 = [(stock_geom, "Stock", "black", 0.05)]

                # Add primary parts with green color
                for part in primary_result.placed_parts:
                    label = f"{config_name}_{part.part_id}"
                    geometries2.append((part.geometry, label, "green", 0.7))

                # Sub-block colors - each sub-block gets a unique color (distinct from green and red)
                sub_block_colors = ["#3498db", "#9b59b6", "#f39c12", "#1abc9c", "#e67e22", "#8e44ad", "#16a085"]

                for i, result in enumerate(sub_block_results):
                    if result is None:
                        # Add empty scrap block in red
                        sb = sub_blocks[i]
                        sb_geom = (cq.Workplane("XY")
                                .box(sb.dimensions[0], sb.dimensions[1], sb.dimensions[2])
                                .translate((sb.x0 + sb.dimensions[0]/2,
                                            ...
                                            sb.z0 + sb.dimensions[2]/2)))
                        scrap_info = scrap_volumes[i]
                        scrap_label = f"EmptyScrap_SB{i+1}_{scrap_info['scrap_volume']:,.0f}mm³"
                        geometries2.append((sb_geom, scrap_label, "red", 0.3))
                        continue
                ...
                # Create title with parts summary and scrap breakdown showing dimensions
                parts_summary = " + ".join([f"{count} {pname}" for pname, count in sorted(parts_by_type.items(), key=lambda x: -x[1])])
                scrap_summary_short = " | ".join([f"SB{sv['sub_block_index']+1}: {sv['sub_block_dims'][0]:.0f}mm*{sv['sub_block_dims'][1]:.0f}mm*{sv['sub_block_dims'][2]:.0f}mm"
                                                  for sv in scrap_volumes])

                filename2 = f"hierarchical_{config_name}_step2_complete"
                title2 = f"Step 2 ({config_name}): {parts_summary} | Waste: {waste_percentage:.1f}% | Scrap: {scrap_summary_short}"
                ...
                if verbose:
                    ...
                    print(f"    Total scrap volume: {total_scrap_volume:,.0f} mm³")
                    print(f"    Parts: {parts_summary}")
                    print(f"    Scrap breakdown by sub-block:")
                    for sv in scrap_volumes:
                        print(f"      Sub-block {sv['sub_block_index']+1}: {sv['scrap_volume']:,.0f} mm³ "
                              f"({sv['scrap_percentage']:.2f}%) - dims: "
                              f"{sv['sub_block_dims'][0]:.1f}×{sv['sub_block_dims'][1]:.1f}×{sv['sub_block_dims'][2]:.1f}")
    ...
    with open(report_file, 'w') as f:
        ...
        for rank, config in enumerate(all_configurations, 1):
            ...
            f.write(f"  Total Scrap Volume: {config['total_scrap_volume']:,.0f} mm³\n")
            ...
            f.write(f"  Scrap breakdown by sub-block:\n")
            for sv in config['scrap_volumes']:
                f.write(f"    Sub-block {sv['sub_block_index']+1}: {sv['scrap_volume']:,.0f} mm³ "
                        f"({sv['scrap_percentage']:.2f}%) - "
                        f"dims: {sv['sub_block_dims'][0]:.1f}×{sv['sub_block_dims'][1]:.1f}×{sv['sub_block_dims'][2]:.1f} mm\n")
            f.write("\n")
